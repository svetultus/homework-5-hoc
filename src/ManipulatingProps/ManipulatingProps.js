...
export const addLoggedInUser = WrappedComponent => {
  return class addLoggedInUser extends React.Component {
    constructor(props) {
      super(props);
    }
    render() {
      ...
      for (let key in this.props) {
        if (key !== 'user') {
          propsGiven[key] = this.props[key];
        }
      }

      propsGiven.user = getLoggedInUser();

      return <WrappedComponent {...propsGiven} />;
    }
  };
};
...
export const withSort = WrappedComponent => {
  return class withSort extends React.Component {
    constructor(props) {
      super(props);
      this.books = Array.from(this.props.books);
    }
    sortString(a, b) {
